...
export const MoviePage = ({ movieId }: { movieId: number }) => {
  ...
  const movieNotWatchedMutation =
    api.watched.removeMovieWatchedById.useMutation();
  const watchedMovie = api.watched.getWatchedMovieById.useQuery({
  ...
  const handleRemoveMovieForUser = async () => {
    ...
    await removeMovieForUser.mutateAsync({
      movieId: Math.floor(movieId),
      userId: session.user.id,
    });
    router.push(`/`);
          ...
          <div className="max-w-xl lg:flex lg:max-w-3xl lg:flex-row lg:gap-8">
            ...
            <div className="flex flex-col gap-4 text-white">
              ...
              <div className="flex grow-0 gap-4">
                ...
                <span
                  onClick={() => handleRemoveMovieForUser()}
